...
const phoneLogic = extractBlock('{isEditingPhone ? (', '</div>\n                    )}');
const emailLogic = extractBlock('{isEditingEmail ? (', '</div>\n                    )}');
const originLogic = extractBlock('{isEditingOrigin ? (', '</div>\n                    )}');
...
const activitiesLogic = extractBlock('{/* The Vertical Line */}', '</div>\n                            )}');
...
let startIndex = originalFile.indexOf('<DialogContent');
let dialogStart = originalFile.indexOf('<Dialog open={!!selectedLead}');
startIndex = originalFile.indexOf('<DialogContent', dialogStart);

let endIndex = originalFile.indexOf('</DialogContent>', startIndex);

if (startIndex === -1 || endIndex === -1) {
  throw new Error("Could not find start/end in original file");
}

const newFileContent = originalFile.substring(0, startIndex) + newModal + originalFile.substring(endIndex + '</DialogContent>'.length);
